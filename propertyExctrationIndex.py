...
- Use the node types listed under "Allowed Nodes" and only nodes listed under "Allowed nodes"
...
Make sure your answer follows jsonformatting and keep it consice, I only want the nodes and properties.
DO NOT UNDER ANY CIRCUMSTANCES USE ANY OTHER NODES THAN THESE:
- Organization
- Law
- Document
- Regulation
...
json_data = """
{
  "nodes": [
    ...
    },
    {
        ...
        "properties": {
           "Jurisdiction": "EU",
            "Reference": "Regulation (EU) 2019/881"
        }
    },
    {
        "id": "60",
        "labels": ["Document"],
        "properties": {
            "Title": "European Union Agency for Cybersecurity (ENISA)"
        }
    },
    {
      "id": "69",
      ...
      "properties": {
        "Type": "ESAs"
        "Responsibility": "Developing draft regulatory technical standards for specifying criteria of joint examination teams. Concluding administrative arrangements with third-country authorities"
      }
  ]
},
"""
...
# Allow the user to choose which chunks to process
start_chunk = int(51)
end_chunk = int(61)

# Ensure chunk indices are valid
start_chunk = max(0, start_chunk)  # Ensure start_chunk is at least 0
end_chunk = min(len(text_chunks), end_chunk)  # Ensure end_chunk is within bounds

context = ""
counter = 0

with open('V2properties52-61.txt', 'a') as file:
    for i in range(start_chunk, end_chunk):
        ...
        context = result
        counter += 1
        print(f"Processed chunk {i + 1} of {len(text_chunks)}")
        file.write(result + "\n")
